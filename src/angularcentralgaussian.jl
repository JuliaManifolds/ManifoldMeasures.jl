...
"""
struct AngularCentralGaussian{M,N,T} <: MeasureBase.ParameterizedMeasure{N}
    manifold::M
    par::NamedTuple{N,T}
end
...
ManifoldsBase.base_manifold(d::AngularCentralGaussian) = getfield(d, :manifold)

function MeasureBase.basemeasure(μ::AngularCentralGaussian)
...
function MeasureBase.logdensity_def(μ::AngularCentralGaussian{<:Any,(:P,)}, x)
    M = ManifoldsBase.base_manifold(μ)
    d = ManifoldsBase.real_dimension(ManifoldsBase.number_system(M))
    s = ManifoldsBase.representation_size(M)
    n, k = length(s) > 1 ? s : (first(s), 1)
    P = μ.P
    logdetx′Px = isone(k) ? real(dot(x, P, x)) : real(logdet(x' * P * x))
    return d * (k * real(logdet(P)) - n * logdetx′Px) / 2
end
function MeasureBase.logdensity_def(μ::AngularCentralGaussian{<:Any,(:L,)}, x)
    M = ManifoldsBase.base_manifold(μ)
    d = ManifoldsBase.real_dimension(ManifoldsBase.number_system(M))
    s = ManifoldsBase.representation_size(M)
    n, k = length(s) > 1 ? s : (first(s), 1)
...
function Base.rand(rng::Random.AbstractRNG, T::Type, d::AngularCentralGaussian)
    p = default_point(d, T)
    return Random.rand!(rng, p, d)
end

function Random.rand!(
    rng::Random.AbstractRNG, p::AbstractArray, d::AngularCentralGaussian{M,(:L,)}
) where {M}
    z = randn!(rng, p)
    return project!(ManifoldsBase.base_manifold(d), p, d.L * z)
end
